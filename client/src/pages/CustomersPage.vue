  ...
  <q-page class="flex flex-center">
    ...
    >
      <q-card style="width: 500px; max-width: 80vw;">
        <q-toolbar>
          <q-toolbar-title v-if="storeCommon.isNew"><span class="text-weight-bold">Add new customer</span></q-toolbar-title>
          <q-toolbar-title v-else><span class="text-weight-bold">Edit customer</span></q-toolbar-title>
        ...
        <q-card-section class="q-pt-lg">
          ...
          >
            <q-input
              ...
              label="Customer code *"
              hint="Unique code to Identify customer"
              lazy-rules
              :rules="[
                val => val !== null && val !== '' || 'Please enter customer code',
              ]"
            />
            <q-input
              outlined
              ...
              lazy-rules
              :rules="[ val => val && val.length > 0 || 'Please type customer name']"
            />
            <q-input
              outlined
...
export default defineComponent({
  ...
  setup() {
    ...
    const getData = async() => {
      try {
        const response = await api.get(storeCommon.baseUrl + 'customers/getcustomers')
    ...
    const AddNew = async () => {
      ...
      await api.post(storeCommon.baseUrl + 'customers/addcustomers', newCustomer)
        .then((res) => {
            $q.notify({
              progress: true,
              message: res.data,
    ...
    const edit = async () => {
      const editedCustomer = {
        customerId: selectedRow.value[0]._id,
        code: formData.value.code,
        name: formData.value.name,
        phone: formData.value.phone,
        address: formData.value.address,
      ...
      await api.put(storeCommon.baseUrl + 'customers/updatecustomers', editedCustomer)
        .then((res) => {
          $q.notify({
                progress: true,
                message: res.data,
    ...
    const remove = async () => {
      const customerId = selectedRow.value[0]._id
      if (selectedRow.value.length =! 0) {
        await api.delete(`${storeCommon.baseUrl}customers/deletecustomers/${customerId}`)
        .then((res) => {
          $q.notify({
              progress: true,
              message: res.data,
